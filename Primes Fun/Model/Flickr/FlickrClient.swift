...
class FlickrClient: NSObject {
    ...
    static func getPhotosAbstracted(completion: @escaping (_ data: [URL]?,
        _ errorReason: UIViewController.AlertReason) -> Void) {
        ...
        let task = session.dataTask(with: request) { (data, response, error) in
            ...
            guard let stat = parsedResult[FlickrConstants.FlickrResponseKeys.Status] as? String,
                stat == FlickrConstants.FlickrResponseValues.OKStatus else {
                completion(nil, .unknown)
                return
            }
            guard let photosDictionary = parsedResult[FlickrConstants.FlickrResponseKeys.Photos]
                as? [String: AnyObject] else {

                completion(nil, .unknown)
                return
            }
            guard let photoArray = photosDictionary[FlickrConstants.FlickrResponseKeys.FlickrPhoto]
                as? [[String: AnyObject]] else {
                completion(nil, .unknown)
                return
            }
            ...
            for _ in 1...70 {
                ...
                guard let imageUrlString = photoDictionary[FlickrConstants.FlickrResponseKeys.MediumURL]
                    as? String else {
                    completion(nil, .unknown)
                    return
                }
    ...
    static func downloadSingleImage( imgUrl: URL,
                                     completion: @escaping (_ imageData: Data?, _ errorString: String?) -> Void) {
        let session = URLSession.shared
        let request: NSURLRequest = NSURLRequest(url: imgUrl)
        let task = session.dataTask(with: request as URLRequest) {data, _, errorString in
            if errorString != nil {
    ...
    static func flickrURLFromParameters() -> URL {
        let parameters = [
            ...
            FlickrConstants.FlickrParameterKeys.APIKey:
                FlickrConstants.FlickrParameterValues.APIKey,
            ...
            FlickrConstants.FlickrParameterKeys.NoJSONCallback:
                FlickrConstants.FlickrParameterValues.DisableJSONCallback,
            FlickrConstants.FlickrParameterKeys.PerPage: FlickrConstants.FlickrParameterValues.PerPage,
            FlickrConstants.FlickrParameterKeys.GroupID:
                FlickrConstants.FlickrParameterValues.GroupID
        ]
